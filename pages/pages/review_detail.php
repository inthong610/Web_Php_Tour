...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">리뷰 관리 페이지</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                	<?php
                                	include '../DAO.php';
                                	$sql = "select member.Name, package.Product , package.Departure , a.rate , a.comment , a.Rid from member, package,(select * from review) as a WHERE a.mid = member.MID AND a.pid = package.PID;"; // 모든 memeber 정보 출력
                                	$ret = mysqli_query($con,$sql) or die(mysql_error());
                                	$numrow = mysqli_num_rows($ret);
                                	while($row = mysqli_fetch_array($ret))
                                	{
                                		$departure = substr("".$row['Departure'] , 0 ,10);
                                		echo ("<tr><td>$row[Name]</td><td>$row[Product]</td><td>$departure</td><td>$row[comment]</td><td>$row[rate]&nbsp;&nbsp;<button class='btn btn-default btn-xs' type='button' onClick='revise_review($row[Rid])'>수정</button></td>");
                                	}
                                	mysqli_close($con);
                                	?>
                                </tbody>
    ...
    function revise_review(rid){
        location.href="review_revise.php?rid="+rid;
    }
    </script>
</body>
</html>
