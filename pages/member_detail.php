                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                	<?php
                                	include '../DAO.php';
                                	$sql = "select * from member;"; // 모든 memeber 정보 출력
                                	$ret = mysqli_query($con,$sql) or die(mysql_error());
                                	$numrow = mysqli_num_rows($ret);
                                	while($row = mysqli_fetch_array($ret))
                                	{
                                		echo ("<tr><td>$row[Name]</td><td>$row[ID]</td><td>$row[Birth]</td><td>$row[Phone]</td><td>$row[Email]&nbsp;&nbsp;<button class='btn btn-default btn-xs' type='button' onClick='delete_user($row[MID])'>삭제</button></td>");
                                	}
                                	mysqli_close($con);
                                	?>
                                </tbody>
